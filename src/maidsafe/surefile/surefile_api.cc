...
#include "maidsafe/surefile/credentials.pb.h"
...
SureFileAPI::SureFileAPI(Slots slots)
...
void SureFileAPI::InsertInput(uint32_t position, const std::string& characters,
...
void SureFileAPI::RemoveInput(uint32_t position, uint32_t length, InputField input_field) {
  switch (input_field) {
    case kPassword: {
      if (!password_)
...
bool SureFileAPI::CanCreateUser() const {
  if (logged_in_)
    return false;
...
void SureFileAPI::CreateUser(const std::string& product_id) {
  if (logged_in_)
    return;
  FinaliseInput(false);
  ConfirmInput();
...
void SureFileAPI::Login(const std::string& product_id) {
  if (logged_in_)
    return;
  FinaliseInput(true);
...
void SureFileAPI::AddService(const std::string& storage_path, const std::string& service_alias) {
  if (!logged_in_)
    ThrowError(CommonErrors::uninitialised);
...
bool SureFileAPI::RemoveService(const std::string& service_alias) {
  if (!logged_in_)
    ThrowError(CommonErrors::uninitialised);
...
bool SureFileAPI::logged_in() const {
  return logged_in_;
}

std::string SureFileAPI::mount_path() const {
...
SureFileAPI::ServiceMap SureFileAPI::service_pairs() const {
  return ReadConfigFile();
}

Slots SureFileAPI::CheckSlots(Slots slots) const {
...
void SureFileAPI::InitialiseService(const std::string& storage_path,
...
void SureFileAPI::FinaliseInput(bool login) {
  if (!password_) {
    if (confirmation_password_)
      ResetConfirmationPassword();
    ThrowError(SureFileErrors::invalid_password);
...
void SureFileAPI::ResetPassword() {
  password_.reset();
}

void SureFileAPI::ResetConfirmationPassword() {
  confirmation_password_.reset();
}

void SureFileAPI::MountDrive(const std::string& product_id, const Identity& drive_root_id) {
...
SureFileAPI::ServiceMap SureFileAPI::ReadConfigFile() const {
  ServiceMap service_pairs;
...
void SureFileAPI::WriteConfigFile(const ServiceMap& service_pairs) const {
...
void SureFileAPI::AddConfigEntry(const std::string& storage_path,
...
void SureFileAPI::OnServiceAdded() const {
  slots_.on_service_added();
}

void SureFileAPI::OnServiceRemoved(const std::string& service_alias) const {
  ServiceMap service_pairs(ReadConfigFile());
...
void SureFileAPI::OnServiceRenamed(const std::string& old_service_alias,
...
void SureFileAPI::PutIds(const fs::path& storage_path,
...
void SureFileAPI::DeleteIds(const fs::path& storage_path) const {
  fs::remove(storage_path / kSureFile);
}

std::pair<Identity, Identity> SureFileAPI::GetIds(const fs::path& storage_path) const {
  crypto::SecurePassword secure_password(SecurePassword());
  crypto::AES256Key key(SecureKey(secure_password));
  crypto::AES256InitialisationVector iv(SecureIv(secure_password));
...
void SureFileAPI::CheckDuplicate(const std::string& storage_path,
                                 const std::string& service_alias) const {
  ServiceMap service_pairs(ReadConfigFile());
  for (const auto& service_pair : service_pairs)
...
bool SureFileAPI::ValidateContent(const std::string& content) const {
  crypto::SecurePassword secure_password(SecurePassword());
  crypto::AES256Key key(SecureKey(secure_password));
  crypto::AES256InitialisationVector iv(SecureIv(secure_password));
...
NonEmptyString SureFileAPI::Serialise(const Identity& drive_root_id,
                                      const Identity& service_root_id) const {
...
std::pair<Identity, Identity> SureFileAPI::Parse(
    const NonEmptyString& serialised_credentials) const {
  protobuf::Credentials credentials;
...
crypto::SecurePassword SureFileAPI::SecurePassword() const {
  return crypto::SecurePassword(crypto::Hash<crypto::SHA512>(password_->string()));
}

crypto::AES256Key SureFileAPI::SecureKey(const crypto::SecurePassword& secure_password) const {
  return crypto::AES256Key(secure_password.string().substr(0, crypto::AES256_KeySize));
}

crypto::AES256InitialisationVector SureFileAPI::SecureIv(
...
std::string SureFileAPI::EncryptSureFile() const {
  crypto::SecurePassword secure_password(SecurePassword());
  crypto::AES256Key key(SecureKey(secure_password));
  crypto::AES256InitialisationVector iv(SecureIv(secure_password));
...
const std::string SureFileAPI::kSureFile("surefile");
